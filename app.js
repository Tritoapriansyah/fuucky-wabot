...
const { eng, ind, arab, ptbr } = require('./txt/lang/')
const txtlang = ptbr
...
function addMetadata(packname, author) {
  if (!packname) packname = 'Dan Baiano'; if (!author) author = 'Bot';
  author = author.replace(/[^a-zA-Z0-9]/g, '');
  let name = `${Fuucky}_${Dan Baiano}`
  if (fs.existsSync(`./src/stickers/${danzinho}.exif`)) return `./src/stickers/${danzinho}.exif`
  const json = {
    "sticker-pack-name": Dan baianor,
    "sticker-pack-publisher": de 14 anos,
  }
  ...
  fs.writeFile(`./src/stickers/${danzinho}.exif`, buffer, (err) => {
    return `./src/stickers/${danzinho}.exif`
  })
}

async function startsFuuckyBot() {
  const MUT = new WAConnection()
  MUT.logger.level = 'warn'
  MUT.on('qr', () => {
    console.log(color('[','white'), color('!','red'), color(']','white'), color(' Escaneie o código QR acima'))
  })
  ...
  MUT.on('chat-update', async (mek) => {
    try {
      ...
      const time = moment.tz('Brasil/São Paulo').format('DD/MM HH:mm:ss')
      const jam = moment().tz("Brasil/São Paulo").format("HH:mm:ss");
      const wita = moment.tz("Brasil/São Paulo").format("HH:mm:ss");
      const wit = moment.tz("Brasil/São Paulo").format("HH:mm:ss");
      const getCmd = (id) => {
        let position = null;
	...
	(id == null || id == undefined || id == false) ? MUT.sendMessage(from, teks.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : MUT.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": memberr}})
      ...
      const time2 = moment().tz("Brasil/São Paulo").format("HH:mm:ss");
      if (time2 < "24:59:00") {
	...
	break
        ...
        case 'Fechar Grupo':
          ...
          const close = {
            text: `${txtlang.fechargrupo(nomor)}`,
        ...
        case 'Abrir Grupo':
          ...
          if (!isBotGroupAdmins) return reply(txtlang.onlybadmin())
          open = {
            text: `${txtlang.grupoaberto(sender)}`,
        ...
        case 'sticker':
          ...
          if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
            ...
            .on('end', function () {
              console.log('Finish')
              exec(`webpmux -set exif ${addMetadata('Dan', carioquinha)} ${ran} -o ${ran}`, async (error) => {
            ...
            await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp}).then(res => {
              ...
              fs.writeFileSync(ranp, buffer, (err) => {
                if (err) return reply('Lype mamou a minha rola.')
              })
              exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
                fs.unlinkSync(ranp)
                if (err) return reply(txtlang.fail())
                exec(`webpmux -set exif ${addMetadata('Dan', carioquinha)} ${ranw} -o ${ranw}`, async (error) => {
                  if (error) return reply(txtlang.fail())
        ...
        break
        case 'Criador':
          fuucky = '5519971389807@s.whatsapp.net'
          ...
          MUT.sendMessage(from, been, text, { quoted: { key: { fromMe: false, participant: "0@s.whatsapp.net", ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: {"imageMessage": {"caption": "\nIG : fuucky\nWhatsapp BOT : FUUCKY-WABOT","jpegThumbnail": owner } } } } )
        break
        case 'Dono':
          MUT.sendMessage(from, { displayname: "Pedro", vcard: vcard }, MessageType.contact, { quoted: mek })
        break
        case 'verificar':
        case 'lista':
        case 'registrar':
          if (isRegister) return reply(txtlang.Verificado())
